...
func NewProductImageHandler(productRepo product.ProductRepository, productImageRepo ProductImageRepository) *ProductImageHandler {
...
// @Success      201 {object} product_image.ProductImageResponse
...
// @Produce      json
// @Param        id path int true "Product ID"
// @Success      200 {array} product_image.ProductImageResponse
// @Failure      400 {object} map[string]string
// @Failure      500 {object} map[string]string
...
func (h *ProductImageHandler) GetProductImages(c echo.Context) error {
	...
	var response []ProductImageResponse
	for _, img := range images {
		response = append(response, ProductImageResponse{
			ID:        img.ID,

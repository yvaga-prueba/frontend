...
func (r *ProductRepo) Read(ctx context.Context) ([]Product, error) {
...
func (r *ProductRepo) GetByID(ctx context.Context, id int64) (Product, error) {
	...
	if errors.Is(err, sql.ErrNoRows) {
		return Product{}, domainerrors.ErrNotFound
	}
	return p, err
}

func (r *ProductRepo) List(ctx context.Context, f ProductFilter) ([]Product, error) {
	q := `
		SELECT id, bar_code, title, description, stock, size, category, unit_price, updated_at, created_at
		FROM products`
	args := []any{}

...
import (
	"context"
	domainerrors "core/internal/errors"
	"database/sql"
	"fmt"
	"log"
)
...
var _ UserRepository = (*SQLRepository)(nil)

func (m *SQLRepository) GetByEmail(ctx context.Context, email string) (res User, err error) {
...
func (m *SQLRepository) GetByID(ctx context.Context, id int64) (res User, err error) {
	query := `SELECT id, first_name, last_name, email, password, google_id, ios_id, provider, role, updated_at, created_at 
	          FROM users 
	          WHERE id = ?`
...
func (m *SQLRepository) GetByGoogleID(ctx context.Context, googleID string) (User, error) {
	query := `SELECT id, first_name, last_name, email, password, google_id, ios_id, provider, role, updated_at, created_at 
	          FROM users 
	          WHERE google_id = ?`
...
func (m *SQLRepository) Store(ctx context.Context, user *User) error {
...
func (m *SQLRepository) Update(ctx context.Context, user *User) error {
	query := `UPDATE users 
...
func (m *SQLRepository) Delete(ctx context.Context, id int64) (err error) {

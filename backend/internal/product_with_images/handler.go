...
type ProductFacadeHandler struct {
	...
	ProductImageHandler *product_image.ProductImageHandler
}
...
func (h *ProductFacadeHandler) CreateProductWithImages(c echo.Context) error {
	...
	var req CreateProductFacadeRequest
	...
	if err != nil {
		...
		return c.JSON(http.StatusCreated, map[string]interface{}{
			"product": product.FromEntity(*createdProduct),
			"images":  []product_image.ProductImageResponse{},
		})
	}
	...
	var imagesResp []product_image.ProductImageResponse
	...
	for i, fh := range files {
		...
		imgEntity := &product_image.ProductImage{
		...
		imagesResp = append(imagesResp, product_image.ProductImageResponse{
			ID:        imgEntity.ID,
